...
class Auth extends \CodeIgniter\Controller {
    ...
    public function __construct() {
        ...
        if (!$this->session->lang):
            $this->session->set('lang', service('request')->getLocale());
        endif;
        $language->setLocale($this->session->lang);

        /*
         * @cardata Hack to get the customized view path in the index page
         * view folder is defined in IonAut config file (app/Config/IonAuth.php)
         */
    ...
    public function create_user() {
        ...
        if ($this->request->getPost() && $this->validation->withRequest($this->request)->run() && $this->ionAuth->register($identity, $password, $email, $additionalData)) {
            ...
            $this->data += [
                ...
                'content' => 'layout/default/content',
                'page_title' => trad('Users'),
                'breadcrumb' => ['/' => trad('Home', 'global'), '/auth' => trad('Users'), '/auth/create_user' => lang('Auth.create_user_heading')]
            ];
    ...
    public function create_group() {
        ...
        if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
            ...
            $this->data += [
                ...
                'content' => 'layout/default/content',
                'page_title' => trad('Users'),
                'breadcrumb' => ['/' => trad('Home', 'global'), '/auth' => trad('Users'), '/auth/create_group' => lang('Auth.create_group_heading')]
            ];
